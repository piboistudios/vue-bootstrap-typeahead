...
<div>
    ...
    <b-row v-else>
        <b-col class="p-0" :cols="Math.round(12 / formattedData.filter(list => list.length > 0).length)" v-for="(col, index) in formattedData" :key="index" v-show="col.length > 0">
            <vue-bootstrap-typeahead-list class="w-100" :title="cols[index].title" :id="`typeahead-input-${index}`" :query="value" :data="col" :background-variant="backgroundVariant" :text-variant="textVariant" v-show="isFocused && col.length > 0" :minMatchingChars="minMatchingChars" @hit="handleHit">
                <!-- pass down all scoped slots -->
                <template v-for="(slot, slotName) in $scopedSlots" :slot="slotName" slot-scope="{ data, htmlText }">
                    <slot :name="slotName" v-bind="{ data, htmlText }"></slot>
                </template>
